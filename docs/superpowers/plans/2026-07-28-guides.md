# 가이드 시스템(MDX 인프라 + 5편) 구현 계획

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** SPEC.md §8의 가이드 5편을 MDX로 구현하고, `/guide` 라우팅과 홈페이지·도구 3페이지와의 상호 링크를 연결한다.

**Architecture:** Next.js App Router의 파일 기반 MDX 페이지(`@next/mdx`) — 가이드마다 독립 폴더(`app/guide/<slug>/page.mdx`), 동적 라우트나 frontmatter 파서 없음. 메타데이터는 `lib/guides.ts` 배열 하나로 관리하고 목록/홈/도구 페이지가 재사용.

**Tech Stack:** Next.js App Router (Server Component), `@next/mdx` + `@mdx-js/loader` + `@mdx-js/react`(신규), Tailwind CSS(기존 토큰).

## Global Constraints

- 서버·DB·외부 API 추가 금지 (CLAUDE.md 규칙 1) — 정적 MDX 페이지라 해당 없음
- 특정 금융상품·종목 추천 금지, 절대 규칙 (CLAUDE.md 규칙 2 / D-02) — 5편(risk-asset-70)은
  비율 규정만 설명하고 상품·종목명을 절대 언급하지 않는다
- 도구 페이지가 아니므로 가이드 페이지에 `<Disclaimer />`/`<AdSlot />` 불필요
- 새 npm 의존성: `@next/mdx`, `@mdx-js/loader`, `@mdx-js/react`(dependencies),
  `@types/mdx`(devDependency) — MDX 콘텐츠 포맷 자체에 필수라 정당화됨(설계 문서 §2), 이
  외의 신규 패키지(contentlayer 등) 추가 금지
- frontmatter 자동 파싱 라이브러리 사용 금지 — 5편뿐이라 `lib/guides.ts`의 정적 배열로 충분
  (YAGNI, 설계 문서 §17)
- 컨테이너 레이아웃에서 `<section>`에는 `px-6`을 넣지 않고 안쪽 `mx-auto max-w-5xl ... px-6`
  요소 하나에만 패딩을 몰아준다 — 분산시키면 데스크톱 폭에서 헤더·푸터와 정렬이 어긋남
  (2026-07-28 홈페이지 작업에서 발견·수정된 버그, 재발 금지)
- 커밋 메시지 접두어: `feat:`, `fix:`, `content:`, `seo:`, 체크리스트 전용 커밋은 `chore:` 허용
- 참조 설계 문서: `docs/superpowers/specs/2026-07-28-guides-design.md`
...
**Files:**
- Modify: `package.json` (npm install로 자동 반영)
- Modify: `next.config.ts`
- Create: `mdx-components.tsx` (프로젝트 루트, `app/`와 같은 레벨)

**Interfaces:**
- Consumes: 없음
- Produces: `.mdx` 파일을 Next.js 페이지로 인식하는 빌드 설정. `useMDXComponents` — 이후
  Task 4~8의 모든 `.mdx` 파일이 마크다운 렌더링 시 자동으로 사용(직접 import 불필요, Next.js
  MDX 통합의 표준 동작).
...
- [ ] **Step 2: `next.config.ts`를 다음으로 교체**
...
- [ ] **Step 3: `mdx-components.tsx` 작성 (프로젝트 루트, `package.json`과 같은 폴더)**
...
- [ ] **Step 4: 타입체크 + 빌드로 MDX 설정 자체 검증**

Run: `npx tsc --noEmit -p tsconfig.json`
Expected: 에러 없음

Run: `npm run build`
Expected: 기존 8개 라우트(`/`, `/about`, `/privacy`, `/contact`, `/tools/*` 3개, `/_not-found`)가
전과 동일하게 `○ (Static)`으로 빌드됨 — 아직 `.mdx` 페이지가 없으므로 새 라우트는 없음, 설정
자체가 기존 빌드를 깨지 않는지만 확인

- [ ] **Step 5: 커밋**

```bash
git add package.json package-lock.json next.config.ts mdx-components.tsx
git commit -m "feat: MDX 인프라 설정 (@next/mdx)"
```

---

## Task 2: `lib/guides.ts` + `components/related-guides.tsx`

**Files:**
- Create: `lib/guides.ts`
- Create: `components/related-guides.tsx`

**Interfaces:**
- Consumes: 없음
- Produces:
  - `export type Guide = { slug: string; title: string; description: string; publishedAt: string }`
  - `export const GUIDES: Guide[]` — Task 3(목록 페이지), Task 9(홈페이지 최신 가이드 섹션)가
    사용
  - `export const RELATED_GUIDES: Record<string, string[]>` — `components/related-guides.tsx`와
    Task 9(도구 페이지)가 사용
  - `export function RelatedGuides({ tool }: { tool: keyof typeof RELATED_GUIDES }): JSX.Element`
    — Task 9의 도구 페이지 3곳이 사용

이 태스크는 Task 4~8(가이드 5편)이 아직 존재하지 않는 상태에서 그 slug 문자열을 참조하는
배열을 만든다 — TypeScript는 문자열 배열의 존재 여부를 라우트와 대조 검증하지 않으므로(이
프로젝트는 `typedRoutes` 미사용) 타입 에러 없이 독립적으로 완료 가능하다.

- [ ] **Step 1: `lib/guides.ts` 작성**
...
- [ ] **Step 2: `components/related-guides.tsx` 작성**
...
```bash
git add lib/guides.ts components/related-guides.tsx
git commit -m "feat: 가이드 메타데이터(lib/guides.ts) + RelatedGuides 컴포넌트 추가"
```

---

## Task 3: `/guide` 목록 페이지 + 레이아웃
...
**Interfaces:**
- Consumes: `GUIDES` from `@/lib/guides` (Task 2)
- Produces: 없음 (Task 4~8의 `.mdx` 파일들이 이 `layout.tsx`의 자식으로 자동 렌더링되지만,
  레이아웃 파일 자체는 자식 라우트가 무엇을 렌더링하는지 알 필요 없음 — Next.js가 자동으로
  `{children}`에 주입)

이 폴더 구조(`app/guide/layout.tsx`)는 이후 만들어질 `app/guide/db-vs-dc/page.mdx` 등
5개 하위 라우트에도 자동으로 적용되는 공통 컨테이너다. Task 4~8을 구현하는 사람은 이 레이아웃이
이미 `max-w-3xl` 컨테이너·패딩·`text-navy`를 제공한다는 것을 알고, 각 `.mdx` 파일 안에서
컨테이너 div를 반복하지 않아야 한다.

- [ ] **Step 1: `app/guide/layout.tsx` 작성**
...
Run: `npx tsc --noEmit -p tsconfig.json`
Expected: 에러 없음. (`npm run build`는 아직 실행하지 않는다 — 이 시점엔 `/guide` 목록이 5개
가이드로 링크를 걸지만 그 라우트들이 아직 없다. 존재하지 않는 내부 링크라도 이 프로젝트는
`typedRoutes`를 쓰지 않으므로 빌드가 깨지지는 않지만, 정식 빌드 검증은 모든 가이드가 갖춰지는
Task 9에서 한 번에 수행한다.)

- [ ] **Step 4: 커밋**

```bash
git add app/guide/layout.tsx app/guide/page.tsx
git commit -m "feat: 가이드 목록 페이지(/guide) 추가"
```

---

## Task 4: 가이드 1편 — `app/guide/db-vs-dc/page.mdx`

**Files:**
- Create: `app/guide/db-vs-dc/page.mdx`

**Interfaces:**
- Consumes: `ToolCTA` from `@/components/tool-cta` (기존 컴포넌트, props:
  `{ title: string; description: string; href: string; ctaLabel: string }`) · `app/guide/layout.tsx`
  (Task 3)가 제공하는 컨테이너(이 파일에서 컨테이너 div를 새로 만들지 않는다)
- Produces: 없음

이 글의 서사(계기·고민·전환 과정·결정 이유·조언)는 운영자 인터뷰 답변을 바탕으로 이미 문장으로
다듬어져 있다 — 아래 내용을 그대로 옮겨적는다(내용을 바꾸거나 새로 창작하지 않는다).

- [ ] **Step 1: `app/guide/db-vs-dc/page.mdx` 작성**
...
```bash
git add app/guide/db-vs-dc/page.mdx
git commit -m "content: 가이드 1편(DB vs DC + 전환 결정 과정) 추가"
```

---

## Task 5: 가이드 2편 — `app/guide/severance-tax-explained/page.mdx`

**Files:**
- Create: `app/guide/severance-tax-explained/page.mdx`

**Interfaces:**
- Consumes: `ToolCTA` from `@/components/tool-cta` · `app/guide/layout.tsx`(Task 3)
- Produces: 없음

이 글의 계산 예시(근속 15년, 퇴직급여 1억 5천만원)는 `lib/calculators/severance-tax.ts`의
실제 계산 함수 로직을 손으로 재현해 검증된 값이다 — 아래 숫자를 그대로 옮겨적는다(다시
계산하거나 반올림 방식을 바꾸지 않는다).

- [ ] **Step 1: `app/guide/severance-tax-explained/page.mdx` 작성**
...
```bash
git add app/guide/severance-tax-explained/page.mdx
git commit -m "content: 가이드 2편(퇴직소득세 계산 구조) 추가"
```

---

## Task 6: 가이드 3편 — `app/guide/lump-vs-pension-guide/page.mdx`

**Files:**
- Create: `app/guide/lump-vs-pension-guide/page.mdx`

**Interfaces:**
- Consumes: `ToolCTA` from `@/components/tool-cta` · `app/guide/layout.tsx`(Task 3)
- Produces: 없음

이 글은 Task 5와 같은 케이스(근속 15년/1.5억)를 10년 연금 수령으로 이어간다. 계산 예시는
`lib/calculators/pension-compare.ts`의 실제 로직(연차별 감면율 적용 후 원단위 절사·누적)을
손으로 재현해 검증된 값이다 — 아래 숫자를 그대로 옮겨적는다.

- [ ] **Step 1: `app/guide/lump-vs-pension-guide/page.mdx` 작성**
...
```bash
git add app/guide/lump-vs-pension-guide/page.mdx
git commit -m "content: 가이드 3편(일시금 vs 연금, 감면 70%/60% 규칙) 추가"
```

---

## Task 7: 가이드 4편 — `app/guide/dc-switch-checklist/page.mdx`

**Files:**
- Create: `app/guide/dc-switch-checklist/page.mdx`

**Interfaces:**
- Consumes: `ToolCTA` from `@/components/tool-cta` · `app/guide/layout.tsx`(Task 3)
- Produces: 없음

- [ ] **Step 1: `app/guide/dc-switch-checklist/page.mdx` 작성**
...
```bash
git add app/guide/dc-switch-checklist/page.mdx
git commit -m "content: 가이드 4편(DC 전환 전 체크리스트) 추가"
```

---

## Task 8: 가이드 5편 — `app/guide/risk-asset-70/page.mdx`

**Files:**
- Create: `app/guide/risk-asset-70/page.mdx`

**Interfaces:**
- Consumes: `ToolCTA` from `@/components/tool-cta` · `app/guide/layout.tsx`(Task 3)
- Produces: 없음

**중요 — Global Constraints 재확인**: 이 글은 D-02 절대 규칙(특정 금융상품·종목 추천 금지)이
가장 크게 걸리는 글이다. 비율 규정(위험자산 70%/안전자산 30%)만 설명하고, 특정 펀드명·ETF
종목명·금융회사명을 어떤 형태로도 언급하지 않는다.

- [ ] **Step 1: `app/guide/risk-asset-70/page.mdx` 작성**
...
```bash
git add app/guide/risk-asset-70/page.mdx
git commit -m "content: 가이드 5편(위험자산 70% 규칙) 추가"
```

---

## Task 9: 홈페이지 "최신 가이드" 섹션 + 도구 3페이지 관련 가이드 링크

**Files:**
- Modify: `app/page.tsx`
- Modify: `app/tools/severance-tax/page.tsx`
- Modify: `app/tools/lump-vs-pension/page.tsx`
- Modify: `app/tools/db-dc/page.tsx`

**Interfaces:**
- Consumes: `GUIDES` from `@/lib/guides`(Task 2), `RelatedGuides` from
  `@/components/related-guides`(Task 2) — 그리고 Task 4~8에서 만든 5개 가이드 라우트가 실제로
  존재해야 링크가 유효하다(이 태스크부터 전체 빌드 검증 수행)
- Produces: 없음

- [ ] **Step 1: `app/page.tsx`에 "최신 가이드" 섹션 추가**

파일 상단 import에 `Link`와 `GUIDES`를 추가한다:
...
`export default function Home()` 안의 두 번째(마지막) `<section>` 바로 다음에 아래 섹션을
추가한다(현재 파일은 `</section>\n    </>\n  );\n}`로 끝난다 — 이 `</section>`과 `</>` 사이에
삽입):
...
`<section>`에는 `px-6`을 넣지 않는다 — 패딩은 안쪽 `mx-auto max-w-5xl ... px-6` div 하나에만
있어야 한다(Global Constraints 참고, 기존 히어로/도구카드 섹션과 동일한 패턴).

- [ ] **Step 2: `app/tools/severance-tax/page.tsx`에 관련 가이드 삽입**

```diff
+import { RelatedGuides } from "@/components/related-guides";
```
을 기존 import들 근처에 추가하고, 마지막 콘텐츠 `<section>`(FAQ 섹션 바로 앞)과 FAQ
`<section>` 사이에 삽입:
...
- [ ] **Step 3: `app/tools/lump-vs-pension/page.tsx`에 관련 가이드 삽입**

동일한 패턴으로 `RelatedGuides` import 추가 후, 마지막 콘텐츠 섹션과 FAQ 섹션 사이에
`<RelatedGuides tool="lump-vs-pension" />`를 삽입한다.

- [ ] **Step 4: `app/tools/db-dc/page.tsx`에 관련 가이드 삽입**

동일한 패턴으로 `RelatedGuides` import 추가 후, 마지막 콘텐츠 섹션과 FAQ 섹션 사이에
`<RelatedGuides tool="db-dc" />`를 삽입한다.
...
```bash
git add app/page.tsx app/tools/severance-tax/page.tsx app/tools/lump-vs-pension/page.tsx app/tools/db-dc/page.tsx
git commit -m "feat: 홈 최신 가이드 섹션 + 도구 3페이지 관련 가이드 링크 연결"
```

---

## Task 10: 전체 빌드 검증 + 회귀 테스트 + 체크리스트 갱신

**Files:**
- Modify: `docs/CHECKLIST.md`

**Interfaces:**
- Consumes: Task 1~9의 모든 산출물 — 이 태스크는 처음으로 전체 구조(MDX 인프라 + 5개 가이드
  라우트 + 목록 페이지 + 상호 링크)가 갖춰진 상태에서 종합 검증한다
- Produces: 없음 (이 계획의 마지막 태스크)

- [ ] **Step 1: 정적 빌드로 전체 라우트 확인**

Run: `npm run build`
Expected: `Route (app)` 표에 다음이 모두 `○ (Static)`으로 표시됨:
`/`, `/about`, `/privacy`, `/contact`, `/guide`, `/guide/db-vs-dc`,
`/guide/severance-tax-explained`, `/guide/lump-vs-pension-guide`, `/guide/dc-switch-checklist`,
`/guide/risk-asset-70`, `/tools/severance-tax`, `/tools/lump-vs-pension`, `/tools/db-dc`,
`/_not-found` — 총 14개 라우트. 에러 없이 빌드 완료되어야 한다.

- [ ] **Step 2: 기존 테스트 스위트 회귀 확인**

Run: `npm test`
Expected: 기존 46개 테스트 전부 `PASS` (이 계획은 `lib/calculators/*`를 건드리지 않음, `lib/guides.ts`는
계산 로직이 없는 정적 데이터라 테스트 대상 아님)

- [ ] **Step 3: Lint 확인**

Run: `npx eslint .`
Expected: 이 계획에서 만든/수정한 파일들에서 에러 0건. `components/lump-vs-pension-calculator.tsx:58`의
기존 무관한 에러는 무시.

- [ ] **Step 4: 개발 서버로 링크·콘텐츠 수동 확인**

`npm run dev` 실행 후 다음을 확인:
- `/guide` 목록에서 5개 가이드 카드가 모두 보이고 각각 클릭 시 올바른 페이지로 이동하는지
- 각 가이드 안의 `<ToolCTA />` 클릭 시 해당 도구 페이지로 이동하는지, 1편·4편 안의
  `[이 가이드](/guide/risk-asset-70)` 마크다운 링크가 정상 작동하는지
- 홈페이지(`/`)의 "최신 가이드" 섹션에 3개 카드가 보이고 "전체 보기 →"가 `/guide`로
  연결되는지
- 도구 3페이지 각각의 FAQ 섹션 바로 위에 "관련 가이드" 3링크가 보이는지
- 375px 모바일 뷰포트에서 가이드 본문(특히 1편처럼 긴 글)이 읽기 편하게 줄바꿈되는지, 목록
  페이지 카드가 세로로 잘 쌓이는지
- 브라우저 콘솔에 에러가 없는지

확인 후 개발 서버 종료.

- [ ] **Step 5: `docs/CHECKLIST.md` 갱신**

`docs/CHECKLIST.md`의 §6에서 다음 4줄을 변경:
...
git commit -m "chore: checklist §6 가이드 시스템 완료 표시"
```
...
- 가이드 8~10편 증량 (PRD Phase 1.5 — 애드센스 신청 전)
- `app/tools/db-dc/page.tsx`, `app/tools/severance-tax/page.tsx`의 기존
  `{/* TODO(운영자): ... 경험담 문단 삽입 예정 */}` 채우기 — 이번 인터뷰 답변으로 채울 수
  있으나 이 계획의 범위(가이드 시스템) 밖, 별도 작업으로 처리
- sitemap.xml — CHECKLIST §7에서 신규 라우트(가이드 5개 포함) 반영해 처리
